...
const PasswordChange = () => {
    ...
    axios.patch(`https://prod.wanted-a.online/users/${id}passwords`,{
            oldPassword: `${oldPassword}`,
            newPassword: `${newPassword}`,
        })
        .then(res => {
            console.log(res.data.result);
            // window.localStorage.getItem(res.data.result.jwt);
            alert('로그인 성공')
            navigate(`/`);
        })
        ...
        <Outline>
            <div className="section">
                <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="password-option" onClick={passwordChange}>비밀번호 초기화/변경</div>
                </form>
            </div>
        </Outline>
    )
